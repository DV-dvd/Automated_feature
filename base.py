...
def evaluate_model(data, estimators, m_features, m_depth):
    ...
    model = RandomForestClassifier(n_estimators=estimators, max_features=m_features, max_depth=m_depth,
                                   min_samples_split=2, min_samples_leaf=1, n_jobs=-1, random_state=0)
    model.fit(x_train, y_train)
    y_pre = model.predict(x_test)
    cnf_matrix = confusion_matrix(y_test, y_pre)
...
def automatically_debugging_options(filename):
    ...
    grid_search = GridSearchCV(estimator=RandomForestClassifier(n_estimators=best_estimator, n_jobs=-1, random_state=0),
                               param_grid=param_test, scoring='roc_auc', verbose=1, n_jobs=-1,
    ...
    grid_search = GridSearchCV(
        estimator=RandomForestClassifier(n_estimators=best_estimator, max_features=best_max_features, n_jobs=-1,
                                         random_state=0),
    ...
    grid_search = GridSearchCV(
        estimator=RandomForestClassifier(n_estimators=best_estimator, max_features=best_max_features,
                                         max_depth=best_max_depth, n_jobs=-1, random_state=0),
        param_grid=param_test, scoring='roc_auc', verbose=1, n_jobs=-1, cv=KFold(3, random_state=0))
    grid_search.fit(X, y)
    ...
    grid_search = GridSearchCV(
        estimator=RandomForestClassifier(n_estimators=best_estimator, max_features=best_max_features,
                                         max_depth=best_max_depth, min_samples_split=best_min_samples_split, n_jobs=-1,
                                         random_state=0),
        param_grid=param_test, scoring='roc_auc', verbose=1, n_jobs=-1, cv=KFold(3, random_state=0))
    grid_search.fit(X, y)
